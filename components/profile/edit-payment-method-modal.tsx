    ...
    try {
      ...
      if (res.data.success) {
        const methods: PaymentMethod = {
          id: String(res.data.result.PmID),
      ...
      alert(
        language === "pt-BR"
          ? "Erro ao editar método de pagamento."
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {language === "pt-BR" ? "Editar Chave Pix" : "Edit Pix Key"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label htmlFor="label">
              {language === "pt-BR" ? "Nome (opcional)" : "Label (optional)"}
            </Label>
            <Input
              id="label"
              value={label}
              ...
              placeholder={
                language === "pt-BR"
                  ? "Ex: Email principal, Celular pessoal..."
                  : "Ex: Main email, Personal phone..."
              }
            />
          </div>
