...
export function ChangePassword({ onBack }: ChangePasswordProps) {
  ...
  const onSubmit = async (data: NewPasswordFormValues) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: language === "pt-BR" ? "Senha alterada com sucesso!" : "Password changed successfully!",
      })
      ...
      setMessage({
        type: "error",
        text: language === "pt-BR" ? "Erro ao alterar senha. Tente novamente." : "Error changing password. Try again.",
      })
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center">
        ...
        <h2 className="text-xl font-semibold">{language === "pt-BR" ? "Alterar Senha" : "Change Password"}</h2>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.confirmPassword && (
              <p className="text-sm text-red-500 mt-1">{t("auth.validation.password.match")}</p>
            )}
          </div>

          {message && <FormMessage type={message.type} message={message.text} />}

          <Button type="submit" disabled={isLoading} className="w-full bg-[#40A578] hover:bg-[#348c65]">
            {isLoading ? (
              <>
